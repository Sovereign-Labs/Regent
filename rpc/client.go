...
func (client *Client) UpdateForkChoice(forkChoice *commands.ForkChoiceState) (*ForkChoiceUpdatedResult, error) {
	return getResponse[*ForkChoiceUpdatedResult](client, NewMessage(FORK_CHOICE_UPDATED, forkChoice), 8*time.Second, &InfiniteRetryStrategy{})
}
...
func (client *Client) UpdateForkChoiceAndBuildBlock(forkChoice *commands.ForkChoiceState, payloadAttributes *commands.PayloadAttributes) (*ForkChoiceUpdatedResult, error) {
	return getResponse[*ForkChoiceUpdatedResult](client, NewMessage(FORK_CHOICE_UPDATED, forkChoice, payloadAttributes), 8*time.Second, &InfiniteRetryStrategy{})
}
...
func (client *Client) SendExecutionPayload(payload *commands.ExecutionPayload) error {
	_, err := getResponse[any](client, NewMessage(NEW_EXECUTION_PAYLOAD, payload), 8*time.Second, &InfiniteRetryStrategy{})
	return err
}
