...
func NewClientWithJwt[R any](port string, secret []byte) Client {
...
func (client *Client) UpdateForkChoice(forkChoice *commands.ForkChoiceState) (*ForkChoiceUpdatedResult, error) {
	return getResponse[*ForkChoiceUpdatedResult](client, NewRequest(FORK_CHOICE_UPDATED, forkChoice), 8*time.Second, &InfiniteRetryStrategy{})
}
...
func (client *Client) UpdateForkChoiceAndBuildBlock(forkChoice *commands.ForkChoiceState, payloadAttributes *commands.PayloadAttributes) (*ForkChoiceUpdatedResult, error) {
	return getResponse[*ForkChoiceUpdatedResult](client, NewRequest(FORK_CHOICE_UPDATED, forkChoice, payloadAttributes), 8*time.Second, &InfiniteRetryStrategy{})
}
...
func (client *Client) SendExecutionPayload(payload *commands.ExecutionPayload) error {
	_, err := getResponse[any](client, NewRequest(NEW_EXECUTION_PAYLOAD, payload), 8*time.Second, &InfiniteRetryStrategy{})
	return err
}
...
func (client *Client) GetPayload(payloadId string) (*commands.ExecutionPayload, error) {
	...
	return getResponse[*commands.ExecutionPayload](client, msg, 1*time.Second, &InfiniteRetryStrategy{})
}
